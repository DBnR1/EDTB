...
if (empty($curSys["coordinates"]) && !empty($curSys["name"]))
{
	if (!isset($_COOKIE["style"]) || $_COOKIE["style"] != "narrow")
	{
		$calc_coord .= "<span style='margin-bottom:6px;height:40px'><a href='javascript:void(0)' onclick='set_reference_systems(false);tofront(\"calculate\");get_cs(\"target_system\")' title='No coordinates found, click here to calculate'>";
	...
	{
		$calc_coord .= "<span style='margin-bottom:6px;text-align:center'><a href='javascript:void(0)' onclick='set_reference_systems(false);tofront(\"calculate\");get_cs(\"target_system\")' title='No coordinates found, click here to calculate'>";
