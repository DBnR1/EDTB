    ...
    if (spgm_IsGallery($strGalleryId)) {
        ...
        if ($strGalleryId != '') {
            ...
            for ($i = 0; $i < $iPathDepth; $i++) {
                // use "foreach ($strConfigurationPathElements as $dir_name) {" in PHP4
...
function spgm_DisplayThumbnailNavibar($iCurrentPageIndex, $iPageNumber, $strGalleryId, $strFilterFlags)
{
    ...
    if ($iCurrentPageIndex > 1) {
        ...
        echo spgm_BuildLink($spgm_cfg['theme']['firstPageIcon'], '', '', $strGalleryId, 1, -1, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags));
        echo '&nbsp; ';
        echo spgm_BuildLink($spgm_cfg['theme']['previousPageIcon'], '', '', $strGalleryId, $iPreviousPageIndex, -1, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags));
    } else {
        echo ' ' . $spgm_cfg['theme']['firstPageIconNot'];
        echo ' &nbsp; ' . $spgm_cfg['theme']['previousPageIconNot'];
    }
    echo ' &nbsp; ';
    ...
    if ($iCurrentPageIndex < $iPageNumber) {
        ...
        echo spgm_BuildLink($spgm_cfg['theme']['nextPageIcon'], '', '', $strGalleryId, $iNextPageIndex, -1, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags));
        echo '&nbsp; ';
        echo spgm_BuildLink($spgm_cfg['theme']['lastPageIcon'], '', '', $strGalleryId, $iPageNumber, -1, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags));
    } else {
        echo ' ' . $spgm_cfg['theme']['nextPageIconNot'];
        echo '  ' . $spgm_cfg['theme']['lastPageIconNot'];
    }
}
...
function spgm_DisplayFilterToggles($strGalleryId, $strFilterFlags, $arrGalleryInfo)
{
    ...
    if (($arrGalleryInfo[1] > 0 && $arrGalleryInfo[0] != $arrGalleryInfo[1]) || $bFilterNewOn) {
        ...
        echo ' &nbsp;&nbsp;<span class="' . CLASS_SPAN_FILTERS . '">[' . $spgm_cfg['locale']['filter'] . ' &raquo; ' . $strHtmlToggles . ']</span>' . "\n";
    }
}
...
function spgm_DisplayGalleryNavibar($strGalleryId, $strFilterFlags, $mixPictureId = '', $arrPictureFilenames)
{
    ...
    if ($spgm_cfg['theme']['gallerySmallIcon'] != '') {
        echo spgm_BuildLink($spgm_cfg['theme']['gallerySmallIcon'], CLASS_TD_GALITEM_TITLE, '', '', -1, -1, $filters);
    } else {
        echo spgm_BuildLink($spgm_cfg['locale']['rootGallery'], CLASS_TD_GALITEM_TITLE, '', '', -1, -1, $filters);
    }
    ...
    if (false !== strpos($strFilterFlags, PARAM_VALUE_FILTER_NEW)) {
        echo ' (' . $spgm_cfg['locale']['newPictures'] . ')';
    }
    ...
    if ($spgm_cfg['conf']['enableSlideshow'] == true) {
        if (false === strpos($strFilterFlags, PARAM_VALUE_FILTER_SLIDESHOW) && count($arrPictureFilenames) > 0) {
            echo ' [';
            echo spgm_BuildLink($spgm_cfg['locale']['filterSlideshow'], CLASS_DIV_GALHEADER, '', $strHtmlGalleryLink, $iCurrentPageIndex, 0, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags) . PARAM_VALUE_FILTER_SLIDESHOW);
            echo ']';
        }
    }
    ...
    if (spgm_IsPicture($strPictureFilename, $strGalleryId)) {
        ...
        if (!$bSlideshowMode) {
            ...
            if ($iPreviousPictureId >= 0) {
                echo spgm_BuildLink($spgm_cfg['theme']['previousPictureIcon'], 'h', ANCHOR_PICTURE, $strGalleryId, -1, $iPreviousPictureId, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags));
            }
            //multi-language support
            ...
            echo ' ' . $spgm_cfg['locale']['pictureNaviBar'] . ' ';

            if ($iNextPictureId < $iPictureNumber) {
                echo spgm_BuildLink($spgm_cfg['theme']['nextPictureIcon'], 'h', ANCHOR_PICTURE, $strGalleryId, -1, $iNextPictureId, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags));
            }
            echo '  </td>' . "\n" . ' </tr>' . "\n";
        }
        ...
        if (count($spgm_cfg['conf']['zoomFactors']) > 0) {
            ...
            for ($i = 0, $iMax = count($spgm_cfg['conf']['zoomFactors']); $i < $iMax; $i++) {
                $iHeight = (int)($arrPictureDim[1] * $spgm_cfg['conf']['zoomFactors'][$i] / 100);
                $iWidth = (int)($arrPictureDim[0] * $spgm_cfg['conf']['zoomFactors'][$i] / 100);
                echo '<input type="button" class="' . CLASS_BUTTON_ZOOM_FACTORS . '" value=" ' . $spgm_cfg['conf']['zoomFactors'][$i] . '% " ';
        ...
        if ($bSlideshowMode) {
            ...
            for ($i = 0; $i < $iPictureNumber; $i++) {
                ...
                echo '  addPicture(\'' . $_strPicturePath . '\', \'' . addslashes($_strPictureCaption) . '\', ' . $_dim[0] . ', ' . $_dim[1] . ');' . "\n";
            }
            echo '</script>' . "\n";
        }
        ...
        if ($spgm_cfg['conf']['filenameWithPictures'] == true) {
            ...
            echo $strPictureBasename . '' . $_strFileExtension . '';
            ...
            if (!file_exists($imgurfile)) {
                ...
                echo '<span id="uploaded" class="right"><a href="' . $imgur_url . '">Link to your image on imgur.com</a><img class="ext_icon" src="/style/img/external_link.png" style="margin-bottom:3px" alt="ext" /></span><br />' . "\n";
            }
        ...
        if (isset($spgm_cfg['captions'][$strPictureFilename])) {
            echo $spgm_cfg['captions'][$strPictureFilename];
        }
...
function spgm_DisplayThumbnails($strGalleryId, $arrPictureFilenames, $iPictureId, $iPageIndex, $strFilterFlags)
{
    ...
    for ($i = $iPictureOffsetStart; $i < $iPictureOffsetStop; $i++) {
        ...
        if ($spgm_cfg['conf']['captionedThumbnails'] == true) {
            if (isset($spgm_cfg['captions'][PREF_THUMB . $strPictureFilename])) {
                ...
                echo $spgm_cfg['captions'][PREF_THUMB . $strPictureFilename];
                echo '</div>' . "\n";
    ...
    if ($iPictureNumber > 0) {
        echo '</tr>' . "\n";
        echo '<tr>' . "\n";
        echo '  <td colspan="' . $spgm_cfg['conf']['thumbnailsPerRow'] . '" class="' . CLASS_TD_THUMBNAILS_NAVI . '">';
...
if (ini_get('register_globals') == '1') {
    $spgm_cfg['global']['documentSelf'] = '/Gallery';
...
if ($spgm_cfg['conf']['filters'] != '') {
    if (!$spgm_cfg['global']['propagateFilters']) {
        if (false !== strpos($spgm_cfg['conf']['filters'], PARAM_VALUE_FILTER_NOTHUMBS) && false === strpos($strParamFilterFlags, PARAM_VALUE_FILTER_NOTHUMBS)) {
            $strParamFilterFlags .= PARAM_VALUE_FILTER_NOTHUMBS;
        }
        if (false !== strpos($spgm_cfg['conf']['filters'], PARAM_VALUE_FILTER_NEW) && false === strpos($strParamFilterFlags, PARAM_VALUE_FILTER_NEW)) {
